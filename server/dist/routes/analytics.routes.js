...
router.get('/overview', async (req, res) => {
    try {
        ...
        // Active Queues
        const activeQueuesRes = await query(`SELECT count(*) as count FROM queues ${organizationId ? "WHERE organization_id = '" + organizationId + "' AND" : 'WHERE'} status = 'ACTIVE'`);
        const activeQueues = parseInt(activeQueuesRes.rows[0].count, 10);
        // Waiting Users
        const waitingRes = await query(`SELECT count(*) as count, 
       ...
       ${organizationId ? 'AND' : 'WHERE'} qe.status = 'WAITING'`);
        ...
        // Active Counters
        const countersRes = await query(`SELECT count(*) as count FROM counters 
       ${organizationId ? "WHERE organization_id = '" + organizationId + "' AND" : 'WHERE'} status IN ('ACTIVE', 'BUSY')`);
        ...
        // Completed Services
        const completedRes = await query(`SELECT count(*) as count FROM queue_entries qe
       ${orgFilterEntry}
       ${organizationId ? 'AND' : 'WHERE'} qe.status = 'COMPLETED'`);
        ...
        const abandonedRes = await query(`SELECT count(*) as count FROM queue_entries qe
       ${orgFilterEntry}
       ${organizationId ? 'AND' : 'WHERE'} qe.status IN ('CANCELLED', 'NO_SHOW')`);
        ...
        const fairnessRes = await query(`SELECT COALESCE(AVG(fairness_score), 95.0) as avg_fairness FROM queue_snapshots`);
...
router.get('/export-csv', async (req, res) => {
    try {
        const result = await query(`SELECT qe.ticket_number, q.name as queue_name, o.name as organization_name,
