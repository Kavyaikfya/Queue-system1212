...
router.post('/simulate-service', async (req, res) => {
    try {
        ...
        const inService = await query(`SELECT * FROM queue_entries WHERE queue_id = $1 AND status = 'IN_SERVICE' LIMIT 1`, [targetQueueId]);
        ...
        if (counterRes.rows.length === 0)
            return res.status(400).json({ error: 'No counter available.' });
        const staffRes = await query(`SELECT id FROM users WHERE role IN ('STAFF', 'ORG_ADMIN') LIMIT 1`);
        const staffId = staffRes.rows[0]?.id || 'demo-staff';
...
router.post('/simulate-noshow', async (req, res) => {
    try {
        ...
        let calledRes = await query(`SELECT * FROM queue_entries WHERE queue_id = $1 AND status = 'CALLED' LIMIT 1`, [targetQueueId]);
        const staffRes = await query(`SELECT id FROM users WHERE role IN ('STAFF', 'ORG_ADMIN') LIMIT 1`);
        const staffId = staffRes.rows[0]?.id || 'demo-staff';
...
router.post('/simulate-counter-fail', async (req, res) => {
    try {
        const { queueId } = req.body;
        const targetQueueId = queueId || (await getDefaultQueueId());
        const activeCounter = await query(`SELECT * FROM counters WHERE queue_id = $1 AND status = 'ACTIVE' LIMIT 1`, [targetQueueId]);
        ...
        await QueueEngine.recalculateAndReorderQueue(targetQueueId, `Counter ${activeCounter.rows[0].counter_number} went OFFLINE`);
        return res.json({
