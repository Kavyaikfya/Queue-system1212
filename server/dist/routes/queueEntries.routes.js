...
router.get('/active', authMiddleware, async (req, res) => {
    try {
        const result = await query(`SELECT qe.*, q.name as queue_name, q.prefix, o.name as organization_name, o.slug as org_slug,
       ...
       ORDER BY qe.join_time DESC`, [req.user.id]);
...
router.get('/history', authMiddleware, async (req, res) => {
    try {
        const result = await query(`SELECT qe.*, q.name as queue_name, o.name as organization_name,
              s.duration_seconds
       FROM queue_entries qe
       JOIN queues q ON qe.queue_id = q.id
       JOIN organizations o ON q.organization_id = o.id
...
router.get('/:id', optionalAuth, async (req, res) => {
    try {
        ...
        const entryRes = await query(`SELECT qe.*, q.name as queue_name, q.prefix, o.name as organization_name, o.type as org_type,
...
router.post('/:id/call', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const entryId = String(req.params.id);
        const { counterId, queueId } = req.body;
        const staffId = req.user.id;
        if (!counterId)
            return res.status(400).json({ error: 'Counter ID is required to call next.' });
        ...
        if (!targetQueueId) {
            ...
            if (ent.rows.length > 0)
                targetQueueId = ent.rows[0].queue_id;
        }
...
router.post('/:id/start', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const entryId = String(req.params.id);
...
router.post('/:id/complete', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const entryId = String(req.params.id);
        const { outcome, notes } = req.body;
        const result = await QueueEngine.completeService(entryId, req.user.id, outcome || 'SUCCESS', notes);
...
router.post('/:id/no-show', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const entryId = String(req.params.id);
...
router.post('/:id/leave', optionalAuth, async (req, res) => {
    try {
        ...
        const entry = entryRes.rows[0];
        await query(`UPDATE queue_entries SET status = 'CANCELLED', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [entryId]);
        await query(`INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, reason, actor_id)
       VALUES (gen_random_uuid(), $1, $2, 'CANCELLED', 'User voluntarily cancelled or left queue.', $3)`, [entry.id, entry.queue_id, req.user?.id || null]);
...
router.post('/:id/urgent', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const entryId = String(req.params.id);
        const entryRes = await query('SELECT * FROM queue_entries WHERE id = $1', [entryId]);
        if (entryRes.rows.length === 0)
        ...
        await QueueEngine.recalculateAndReorderQueue(entry.queue_id, newUrgentState ? 'Urgent triage flag applied' : 'Urgent triage flag cleared', req.user?.id);
        return res.json({
