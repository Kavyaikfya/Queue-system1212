...
import { authMiddleware, optionalAuth, resolveUserFromRequest, } from '../middleware/auth.js';
const router = Router();
// ========================================================
...
router.get('/requirements/:serviceId', optionalAuth, async (req, res) => {
    try {
        ...
        // Fetch service info
        const serviceRes = await query(`SELECT s.*, o.name as organization_name, o.type as org_type
       FROM services s
       ...
       WHERE s.id = $1`, [serviceId]);
        ...
        let reqRes = await query(`SELECT * FROM service_requirements WHERE service_id = $1 ORDER BY created_at ASC`, [serviceId]);
...
router.get('/can-avoid/:serviceId', optionalAuth, async (req, res) => {
    try {
        const { serviceId } = req.params;
        const serviceRes = await query(`SELECT s.*, o.name as organization_name
       FROM services s
       JOIN organizations o ON s.organization_id = o.id
       WHERE s.id = $1`, [serviceId]);
        if (serviceRes.rows.length === 0) {
...
router.get('/readiness/:serviceId', optionalAuth, async (req, res) => {
    try {
        const { serviceId } = req.params;
        const user = await resolveUserFromRequest(req);
        const userId = user?.id || 'guest-user';
        // Check if user already has saved readiness checklist
        const readinessRes = await query(`SELECT * FROM visit_readiness WHERE user_id = $1 AND service_id = $2`, [userId, serviceId]);
        ...
        const initialChecklist = requirements.map((r, idx) => ({
            id: r.id || `req-${idx + 1}`,
            title: r.title,
            category: r.category,
...
router.post('/readiness/:serviceId/toggle', optionalAuth, async (req, res) => {
    try {
        const { serviceId } = req.params;
        ...
        if (readinessRes.rows.length > 0) {
            ...
            const reqRes = await query(`SELECT * FROM service_requirements WHERE service_id = $1`, [serviceId]);
            const requirements = reqRes.rows.length > 0 ? reqRes.rows : getDefaultRequirementsForService(service);
            checklist = requirements.map((r, idx) => ({
                id: r.id || `req-${idx + 1}`,
                title: r.title,
                category: r.category,
                description: r.description,
        ...
        if (readinessRes.rows.length > 0) {
            await query(`UPDATE visit_readiness 
         SET readiness_score = $1, checklist_json = $2, updated_at = CURRENT_TIMESTAMP
         WHERE user_id = $3 AND service_id = $4`, [score, JSON.stringify(checklist), userId, serviceId]);
        }
        else {
            await query(`INSERT INTO visit_readiness (id, user_id, service_id, readiness_score, checklist_json)
         VALUES ($1, $2, $3, $4, $5)`, [uuidv4(), userId, serviceId, score, JSON.stringify(checklist)]);
        }
        return res.json({
            readinessScore: score,
            checklist,
...
router.post('/documents/pre-check', optionalAuth, async (req, res) => {
    try {
        ...
        const isImageOrPdf = lower.endsWith('.pdf') ||
            lower.endsWith('.jpg') ||
            lower.endsWith('.jpeg') ||
            lower.endsWith('.png') ||
        ...
        const docId = uuidv4();
        try {
            await query(`INSERT INTO documents (id, user_id, service_id, file_name, file_size, mime_type, status, precheck_results_json)
...
router.get('/documents', optionalAuth, async (req, res) => {
    try {
        ...
        const docs = await query(`SELECT * FROM documents WHERE user_id = $1 ORDER BY created_at DESC LIMIT 20`, [user.id]);
...
router.get('/facilities', async (_req, res) => {
    ...
    `);
      ...
      ORDER BY s.name ASC
    `);
        const facilities = orgsRes.rows.map((org) => {
            ...
            if (waiting <= 2)
                ...
                demandStatus = 'VERY BUSY';
            const orgServices = servicesRes.rows.filter((s) => s.organization_id === org.id);
            return {
                ...org,
                demandStatus,
...
router.get('/favorites', authMiddleware, async (req, res) => {
    try {
        const result = await query(`SELECT ff.*, o.name as organization_name, o.type as org_type, o.address, o.working_hours
       FROM facility_favorites ff
       JOIN organizations o ON ff.organization_id = o.id
       WHERE ff.user_id = $1
       ORDER BY ff.created_at DESC`, [req.user.id]);
...
router.post('/favorites/toggle', authMiddleware, async (req, res) => {
    try {
        ...
        const existing = await query(`SELECT * FROM facility_favorites WHERE user_id = $1 AND organization_id = $2`, [req.user.id, organizationId]);
        if (existing.rows.length > 0) {
            await query(`DELETE FROM facility_favorites WHERE user_id = $1 AND organization_id = $2`, [req.user.id, organizationId]);
            return res.json({ isFavorite: false, message: 'Removed from favorites.' });
        }
        else {
            await query(`INSERT INTO facility_favorites (id, user_id, organization_id) VALUES ($1, $2, $3)`, [uuidv4(), req.user.id, organizationId]);
...
router.post('/feedback', optionalAuth, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO visit_feedback (id, queue_entry_id, user_id, rating, feedback_text, waiting_duration_minutes, service_duration_minutes)
...
router.get('/search', async (req, res) => {
    try {
        ...
        const [orgs, servs, queues] = await Promise.all([
            query(`SELECT id, name, type, address, working_hours
         FROM organizations
         WHERE lower(name) LIKE $1 OR lower(type) LIKE $1 OR lower(address) LIKE $1
         LIMIT 6`, [searchTerm]),
            query(`SELECT s.id, s.name, s.category, s.avg_duration_minutes, o.name as organization_name
         FROM services s
         JOIN organizations o ON s.organization_id = o.id
         WHERE lower(s.name) LIKE $1 OR lower(s.category) LIKE $1
         LIMIT 6`, [searchTerm]),
            query(`SELECT q.id, q.name, q.prefix, q.status, o.name as organization_name,
                (SELECT count(*) FROM queue_entries qe WHERE qe.queue_id = q.id AND qe.status = 'WAITING') as waiting_count
         FROM queues q
...
function determineOnlineAvailability(service, requirements) {
    ...
    if (cat === 'REGISTRATION' || name.includes('enrollment') || name.includes('tax') || name.includes('deposit')) {
        return {
            canAvoidVisit: true,
            onlineStatus: 'ONLINE OPTION AVAILABLE',
            onlineOptionUrl: 'https://citizenportal.gov/services',
            recommendation: 'This service can be completed online via the organization digital portal. You do not need to attend physically unless biometrics are required.',
        };
    }
    if (cat === 'EMERGENCY' || name.includes('triage') || name.includes('hardware') || name.includes('biometric')) {
        return {
            canAvoidVisit: false,
            onlineStatus: 'PHYSICAL VISIT REQUIRED',
            onlineOptionUrl: null,
            recommendation: 'Physical attendance is required for this service due to in-person diagnostic, biometric, or triage protocol.',
        };
    }
    return {
        canAvoidVisit: false,
        onlineStatus: 'CONTACT FACILITY',
        onlineOptionUrl: null,
        recommendation: 'Preliminary documents may be submitted digitally, but counter attendance is subject to triage verification.',
    };
}
// Helper: Generate smart default requirements
function getDefaultRequirementsForService(service) {
    const cat = (service.category || '').toUpperCase();
    return [
        {
            id: 'req-id-proof',
