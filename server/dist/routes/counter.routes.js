...
router.post('/', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN'), async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, 'ACTIVE')`, [counterId, organizationId, queueId || null, name, counterNumber]);
...
// PUT /counters/:id
router.put('/:id', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
       ...
       WHERE id = $4`, [status, currentStaffId, name, req.params.id]);
        ...
        if (counter.queue_id && status && status !== counter.status) {
            await QueueEngine.recalculateAndReorderQueue(counter.queue_id, `Counter ${counter.counter_number} status changed to ${status}`, req.user?.id);
        }
