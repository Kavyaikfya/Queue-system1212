...
router.get('/services', optionalAuth, async (req, res) => {
    ...
    `;
        ...
        const services = await Promise.all(result.rows.map(async (srv) => {
            const reqRes = await query(`SELECT document_name, is_mandatory, description FROM service_documents WHERE service_id = $1`, [srv.id]);
            ...
            if (srv.queue_id) {
                const qCountRes = await query(`SELECT COUNT(*) as count FROM queue_entries WHERE queue_id = $1 AND status = 'WAITING'`, [srv.queue_id]);
...
router.put('/services/:id/availability', optionalAuth, async (req, res) => {
    try {
        ...
        const now = new Date();
        await query(`INSERT INTO service_availability (id, service_id, status, staff_status, reason, expected_recovery, expected_wait_minutes, last_updated)
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         last_updated = EXCLUDED.last_updated`, [availId, id, status, staffStatus || 'Active', reason || null, expectedRecovery || null, expectedWait || 15, now]);
...
router.post('/snap-understand', optionalAuth, async (req, res) => {
    try {
        ...
        try {
            const matchRes = await query(`SELECT id FROM services WHERE name ILIKE $1 OR category ILIKE $2 LIMIT 1`, [`%${analysis.matchedServiceName}%`, `%${analysis.matchedCategory}%`]);
...
router.get('/passport', optionalAuth, async (req, res) => {
    try {
        ...
        const passportRes = await query(`SELECT * FROM service_passport WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
        // Fetch past consent grants
        const consentsRes = await query(`SELECT sc.*, s.name as service_name, o.name as org_name
       FROM service_consents sc
       JOIN services s ON sc.service_id = s.id
       JOIN organizations o ON s.organization_id = o.id
       WHERE sc.user_id = $1 ORDER BY sc.consent_given_at DESC`, [userId]);
...
router.get('/proxies', optionalAuth, async (req, res) => {
    try {
        const userId = req.user?.id || 'demo_user';
        const proxiesRes = await query(`SELECT sp.*, s.name as service_name
       FROM service_proxies sp
       LEFT JOIN services s ON sp.service_id = s.id
       WHERE sp.user_id = $1 ORDER BY sp.created_at DESC`, [userId]);
...
router.post('/proxies', optionalAuth, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO service_proxies (id, user_id, proxy_name, proxy_relationship, proxy_phone, service_id, allowed_actions, denied_actions, purpose, permission_type, expires_at, status)
...
router.get('/group-bookings', optionalAuth, async (req, res) => {
    try {
        const userId = req.user?.id || 'demo_user';
        const bookingsRes = await query(`SELECT gb.*, s.name as service_name, o.name as org_name
       FROM group_bookings gb
       JOIN services s ON gb.service_id = s.id
       JOIN organizations o ON s.organization_id = o.id
       WHERE gb.creator_user_id = $1 ORDER BY gb.created_at DESC`, [userId]);
...
router.post('/group-bookings', optionalAuth, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO group_bookings (id, service_id, title, creator_user_id, status, scheduled_time)
       VALUES ($1, $2, $3, $4, 'PENDING', $5)`, [bookingId, serviceId, title, userId, new Date(scheduledTime || Date.now() + 48 * 3600 * 1000)]);
        if (Array.isArray(members)) {
            for (const m of members) {
                await query(`INSERT INTO group_members (id, group_booking_id, member_name, role_title, phone, status)
           VALUES ($1, $2, $3, $4, $5, $6)`, [`gm_${uuidv4().slice(0, 8)}`, bookingId, m.name, m.role, m.phone || null, m.status || 'PENDING']);
            }
        }
...
router.get('/opportunity-rescue', optionalAuth, async (req, res) => {
    try {
        const userId = req.user?.id || 'demo_user';
        const rescuesRes = await query(`SELECT * FROM opportunity_recovery WHERE user_id = $1 AND status = 'OFFERED' ORDER BY created_at DESC`, [userId]);
...
router.get('/verifications/reusable', optionalAuth, async (req, res) => {
    try {
        const userId = req.user?.id || 'demo_user';
        const verifRes = await query(`SELECT * FROM verification_records WHERE user_id = $1 AND status = 'VALID'`, [userId]);
...
router.post('/verifications/reuse', optionalAuth, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO process_reuse_records (id, user_id, service_id, verification_id, reused_at, status)
       VALUES ($1, $2, $3, $4, NOW(), 'APPROVED')`, [reuseId, userId, serviceId || 'default', verificationId]);
        return res.json({
...
router.get('/active-services', optionalAuth, async (req, res) => {
    try {
        ...
        const ticketsRes = await query(`SELECT qe.*, q.name as queue_name, q.prefix, s.name as service_name, s.category as service_category,
       ...
       ORDER BY qe.join_time DESC`, [userId]);
        // 2. Fetch active group bookings
        const groupsRes = await query(`SELECT gb.*, s.name as service_name, o.name as organization_name
       FROM group_bookings gb
       JOIN services s ON gb.service_id = s.id
       JOIN organizations o ON s.organization_id = o.id
       WHERE gb.creator_user_id = $1 AND gb.status IN ('PENDING', 'CONFIRMED', 'AT_RISK')`, [userId]);
        return res.json({
...
router.get('/alerts', optionalAuth, async (req, res) => {
    try {
        ...
        const notifsRes = await query(`SELECT * FROM notifications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 10`, [userId]);
