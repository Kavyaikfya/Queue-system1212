...
router.get('/:id', async (req, res) => {
    try {
        const queueRes = await query(`SELECT q.*, o.name as organization_name, o.type as organization_type, s.name as service_name, s.avg_duration_minutes
       FROM queues q
       JOIN organizations o ON q.organization_id = o.id
       LEFT JOIN services s ON q.service_id = s.id
       WHERE q.id = $1`, [req.params.id]);
        ...
        const waitingEntries = await query(`SELECT qe.*, u.full_name as user_name, u.email as user_email
       FROM queue_entries qe
       LEFT JOIN users u ON qe.user_id = u.id
       WHERE qe.queue_id = $1 AND qe.status = 'WAITING'
       ORDER BY qe.position ASC`, [req.params.id]);
        // Fetch in-service entries
        const inServiceEntries = await query(`SELECT qe.*, c.name as counter_name, c.counter_number, u.full_name as staff_name
       FROM queue_entries qe
...
router.post('/:id/pause', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        ...
        await QueueEngine.recalculateAndReorderQueue(queueId, `Queue status updated to ${newStatus}`, req.user?.id);
...
router.post('/:id/reorder', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN', 'STAFF'), async (req, res) => {
    try {
        const queueId = String(req.params.id);
        await QueueEngine.recalculateAndReorderQueue(queueId, 'Manual admin reorder trigger', req.user?.id);
...
router.put('/:id', authMiddleware, requireRole('SUPER_ADMIN', 'ORG_ADMIN'), async (req, res) => {
    try {
       ...
       WHERE id = $6`, [
        ...
        await QueueEngine.recalculateAndReorderQueue(queueId, 'Queue configuration updated by admin');
