...
export class FairnessEngine {
    static DISCLAIMER = 'System-generated fairness indicator based on configured queue rules.';
    static evaluate(entries) {
        ...
        const variance = waitTimes.reduce((acc, v) => acc + Math.pow(v - avgWaitMinutes, 2), 0) /
            totalWaiting;
        ...
        const maxBackwardShifts = Math.max(0, ...entries.map((e) => e.backwardShiftCount || 0));
        ...
        if (maxWaitMinutes > 45) {
            ...
            alerts.push({
                ...
                recommendation: 'Consider assigning an additional counter or expediting standard tickets.',
            });
        }
        ...
        if (stdDev > 20 && totalWaiting >= 4) {
            ...
            alerts.push({
                ...
                severity: 'WARNING',
                message: 'Waiting-time imbalance detected. High variance between fastest and slowest advancing entries.',
                recommendation: 'Check priority rule weights to ensure aging factors prevent newer urgent tickets from stalling earlier arrivals.',
            });
        }
        ...
        if (maxBackwardShifts >= 2) {
            ...
            alerts.push({
                ...
                recommendation: 'Queue priority configuration may require review. Activate starvation threshold limits.',
            });
        }
        // 4. Urgent saturation
        if (urgentRatio > 0.4 && totalWaiting >= 5) {
            ...
            alerts.push({
                ...
                recommendation: 'Dedicate a separate emergency express desk to isolate urgent workflows.',
            });
        }
