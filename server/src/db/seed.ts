...
export async function seedDatabase() {
  ...
  await query(
    `INSERT INTO users (id, email, password_hash, full_name, role, phone)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [superAdminId, 'superadmin@fairqueue.io', hashedPasswordAdmin, 'System Administrator', 'SUPER_ADMIN', '+1-800-555-0100']
  );
  ...
  for (const u of sampleUsers) {
    ...
    await query(
      `INSERT INTO users (id, email, password_hash, full_name, role, phone)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [uId, u.email, hashedPasswordUser, u.name, 'USER', u.phone]
    );
  }
  ...
  for (const orgData of orgConfigs) {
    const orgId = uuidv4();
    await query(
      `INSERT INTO organizations (id, name, slug, type, address, contact, working_hours)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [orgId, orgData.name, orgData.slug, orgData.type, orgData.address, orgData.contact, orgData.working_hours]
    );
    ...
    await query(
      `INSERT INTO users (id, email, password_hash, full_name, role, phone)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [adminId, orgData.admin.email, hashedPasswordAdmin, orgData.admin.name, 'ORG_ADMIN', '+1-555-9000']
    );
    await query(
      `INSERT INTO organization_members (id, organization_id, user_id, role, title)
       VALUES ($1, $2, $3, $4, $5)`,
      [uuidv4(), orgId, adminId, 'ORG_ADMIN', 'Managing Director']
    );
    ...
    await query(
      `INSERT INTO users (id, email, password_hash, full_name, role, phone)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [staffId, orgData.staff.email, hashedPasswordUser, orgData.staff.name, 'STAFF', '+1-555-9001']
    );
    await query(
      `INSERT INTO organization_members (id, organization_id, user_id, role, title)
       VALUES ($1, $2, $3, $4, $5)`,
      [uuidv4(), orgId, staffId, 'STAFF', 'Senior Service Representative']
    );
    ...
    for (const s of orgData.services) {
      ...
      serviceIds.push(sId);
      await query(
        `INSERT INTO services (id, organization_id, name, category, avg_duration_minutes, base_priority)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [sId, orgId, s.name, s.category, s.duration, s.priority]
      );
    }
    ...
    await query(
      `INSERT INTO queues (id, organization_id, service_id, name, prefix, status, max_capacity, operating_hours, priority_rules_json)
    ...
    // Create Counters
    let firstCounterId: string | null = null;
    for (const c of orgData.counters) {
      const cId = uuidv4();
      if (!firstCounterId) firstCounterId = cId;
      await query(
        `INSERT INTO counters (id, organization_id, queue_id, name, counter_number, status, current_staff_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [cId, orgId, queueId, c.name, c.number, c.status, c.number === 1 ? staffId : null]
      );

      if (c.number === 1) {
        await query(
          `INSERT INTO staff_assignments (id, staff_id, counter_id, queue_id, is_active)
           VALUES ($1, $2, $3, $4, TRUE)`,
          [uuidv4(), staffId, cId, queueId]
        );
      }
    }
    ...
    // Priority Rules
    await query(
      `INSERT INTO priority_rules (id, organization_id, queue_id, rule_name, rule_type, weight, condition_json)
       VALUES 
       ...
       ($5, $2, $3, 'Confirmed Appointment Bonus', 'APPOINTMENT_STATUS', 15.00, '{"flag": "has_appointment"}')`,
      [uuidv4(), orgId, queueId, uuidv4(), uuidv4()]
    );
  ...
  try {
    ...
    if (parseInt(reqCheck.rows[0].count, 10) === 0) {
      ...
      for (const s of allServices.rows) {
        ...
        for (const r of reqs) {
          await query(
            `INSERT INTO service_requirements (id, service_id, title, category, description, is_mandatory, can_avoid_visit)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`,
            [uuidv4(), s.id, r.title, r.category, r.desc, r.mandatory, r.online]
          );
        }
      }
