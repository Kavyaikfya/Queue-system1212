...
// POST /organizations
router.post('/', authMiddleware, requireRole('SUPER_ADMIN'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await query(
      ...
      [orgId, name, slug, type, address || null, contact || null, workingHours || '09:00 - 18:00']
    );
    ...
    await query(
      `INSERT INTO services (id, organization_id, name, category, avg_duration_minutes, base_priority)
       VALUES ($1, $2, 'General Assistance', 'STANDARD', 15, 10)`,
      [serviceId, orgId]
    );

    const queueId = uuidv4();
    await query(
      `INSERT INTO queues (id, organization_id, service_id, name, prefix, status, max_capacity)
       VALUES ($1, $2, $3, $4, $5, 'ACTIVE', 100)`,
      [queueId, orgId, serviceId, `${name} Main Queue`, type.charAt(0)]
    );

    // Create initial counter
    await query(
      `INSERT INTO counters (id, organization_id, queue_id, name, counter_number, status)
       VALUES ($1, $2, $3, 'Counter 1', 1, 'ACTIVE')`,
      [uuidv4(), orgId, queueId]
    );
  ...
  try {
    ...
    await query(
      `UPDATE organizations 
       ...
       WHERE id = $5`,
      [name, address, contact, working_hours, req.params.id]
    );
