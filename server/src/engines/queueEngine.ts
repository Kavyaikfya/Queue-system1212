...
export class QueueEngine {
   ...
   */
  public static async recalculateAndReorderQueue(
    queueId: string,
    ...
    const queueRes = await query(
      `SELECT q.*, s.avg_duration_minutes, s.base_priority, s.category as service_category
       FROM queues q
    ...
    const counterRes = await query(
      `SELECT count(*) as count FROM counters 
       WHERE queue_id = $1 AND status IN ('ACTIVE', 'BUSY')`,
      [queueId]
    );
    ...
    for (let index = 0; index < scoredEntries.length; index++) {
      ...
      if (oldPos !== newPos) {
        ...
        await query(
          `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, previous_position, new_position, reason, actor_id)
           VALUES ($1, $2, $3, 'POSITION_CHANGED', $4, $5, $6, $7)`,
          [uuidv4(), entry.id, queueId, oldPos, newPos, causalReason, actorId || null]
        );

        // Notify user if registered
        if (entry.user_id) {
          await query(
            `INSERT INTO notifications (id, user_id, queue_entry_id, title, message, type)
      ...
      await query(
        `UPDATE queue_entries 
         ...
         WHERE id = $5`,
        [newPos, entry.newPriorityScore, entry.newPriorityReason, waitResult.estimatedMinutes, entry.id]
      );
    }
    ...
    if (fairnessReport.alerts.length > 0) {
      for (const alert of fairnessReport.alerts) {
        if (alert.severity === 'WARNING' || alert.severity === 'CRITICAL') {
          await query(
            `INSERT INTO fairness_events (id, organization_id, queue_id, alert_type, severity, explanation, metric_snapshot)
  ...
  private static async saveSnapshot(
    ...
    await query(
      `INSERT INTO queue_snapshots (id, queue_id, queue_length, waiting_count, fairness_score, active_counters)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [uuidv4(), queueId, queueLength, waitingCount, fairnessScore, activeCounters]
    );
  }

  public static broadcastQueueUpdate(queueId: string) {
  ...
  public static async joinQueue(params: {
    ...
    const queueRes = await query(
      `SELECT q.*, count(qe.id) as current_count 
       FROM queues q
    ...
    if (params.userId) {
      const existingRes = await query(
        `SELECT id, ticket_number, position, estimated_wait_minutes, status
         FROM queue_entries
         WHERE user_id = $1 AND queue_id = $2 AND status IN ('WAITING', 'CALLED', 'IN_SERVICE')
         LIMIT 1`,
        [params.userId, params.queueId]
      );
      if (existingRes.rows.length > 0) {
        const existing = existingRes.rows[0];
        const err: any = new Error('You are already in this queue.');
    ...
    const tokenRes = await query(
      `UPDATE queues 
       ...
       WHERE id = $1 
       RETURNING current_token_number, prefix`,
      [params.queueId]
    );
    ...
    await query(
      `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, previous_position, new_position, reason)
       VALUES ($1, $2, $3, 'JOINED', $4, $4, 'Ticket issued. Position assigned in queue.')`,
      [uuidv4(), entryId, params.queueId, initialPos]
    );
   ...
   */
  public static async callNext(queueId: string, counterId: string, staffId: string) {
    ...
    await query(
      `UPDATE queue_entries 
       SET status = 'CALLED',
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $4`,
      [counterId, staffId, noShowDeadline, nextEntry.id]
    );
   ...
   */
  public static async respondTurn(entryId: string, action: 'READY' | 'NEED_TIME', userId?: string) {
    const entryRes = await query(`SELECT * FROM queue_entries WHERE id = $1`, [entryId]);
    if (entryRes.rows.length === 0) throw new Error('Ticket entry not found.');
    ...
    if (action === 'READY') {
      ...
      await query(
        `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, reason, actor_id)
         VALUES ($1, $2, $3, 'POSITION_CHANGED', 'User confirmed ready for service at counter.', $4)`,
        [uuidv4(), entry.id, entry.queue_id, userId || null]
      );
      ...
      await query(
        `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, previous_position, new_position, reason, actor_id)
         VALUES ($1, $2, $3, 'POSITION_CHANGED', 0, 3, 'User requested more time. Postponed by 2 spots without losing ticket.', $4)`,
        [uuidv4(), entry.id, entry.queue_id, userId || null]
      );
   ...
   */
  public static async startService(entryId: string, staffId: string) {
    const entryRes = await query(`SELECT * FROM queue_entries WHERE id = $1`, [entryId]);
    ...
    await query(
      `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, reason, actor_id)
       VALUES ($1, $2, $3, 'SERVICE_STARTED', 'Service initiated at counter.', $4)`,
      [uuidv4(), entryId, entry.queue_id, staffId]
    );
   ...
   */
  public static async completeService(entryId: string, staffId: string, outcome: string = 'SUCCESS', notes?: string) {
    const entryRes = await query(`SELECT * FROM queue_entries WHERE id = $1`, [entryId]);
    if (entryRes.rows.length === 0) throw new Error('Entry not found.');
    const entry = entryRes.rows[0];
    ...
    if (entry.counter_id) {
      await query(
        `INSERT INTO service_sessions (id, queue_entry_id, counter_id, staff_id, start_time, end_time, duration_seconds, notes, outcome)
         VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, $6, $7, $8)`,
        [uuidv4(), entryId, entry.counter_id, staffId, startTime, durationSeconds, notes || null, outcome]
      );
    ...
    await query(
      `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, reason, actor_id)
       VALUES ($1, $2, $3, 'SERVICE_COMPLETED', 'Service fulfilled and ticket completed.', $4)`,
      [uuidv4(), entryId, entry.queue_id, staffId]
    );
   ...
   */
  public static async markNoShow(entryId: string, staffId: string, reason: string = 'Did not respond within timer') {
    const entryRes = await query(`SELECT * FROM queue_entries WHERE id = $1`, [entryId]);
    if (entryRes.rows.length === 0) throw new Error('Entry not found.');
    const entry = entryRes.rows[0];
    ...
    await query(
      `INSERT INTO queue_events (id, queue_entry_id, queue_id, event_type, reason, actor_id)
       VALUES ($1, $2, $3, 'NO_SHOW', $4, $5)`,
      [uuidv4(), entryId, entry.queue_id, reason, staffId]
    );
